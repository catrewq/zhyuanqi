...
import { Button, Form, Input, message, Space, Table, Upload, Modal } from 'antd';
...
const List = () => {
  ...
  const handleUpload = async (file) => {
    ...
    if (data.failedFileUrl) {
      ...
      if ((data.success === false || data.failedCount > 0) && data.message) {
        message.error(data.message);
      }
      query();
    }
        ...
        return (
          <Space>
            ...
            <Button
              type="link"
              size="small"
              onClick={() => { showInfoModal(record.id) }}
            >
              设置信息
            </Button>
          </Space>
  ...
  return (
    <div className="list-page">
      ...
      <SetInfo
        id={currentId}
        showModal={(obj) => showInfoModal(obj)}
        visible={InfoVisible}
      />
    </div>
  );
